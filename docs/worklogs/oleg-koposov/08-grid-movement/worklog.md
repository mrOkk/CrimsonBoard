...
| `updated_at` | `2026-05-24T19:29:35Z`|

## Phase History

| When | From → To | Note |
|---|---|---|
| 2026-05-24T19:29:35Z | active → done | Implemented by wf-implement |
...
- [x] [Task 2: GridMovementSystem — MoveResult, TryMove, OccupancyMap, HealthSystem](tasks/task-2.md)
- [x] [Task 3: PlayerMovementSystem — input, cooldown, 8-dir, direction indicator](tasks/task-3.md)
- [x] [Task 4: Wiring — PlayerSpawnSystem, HealthSystem knockback, GameplayState](tasks/task-4.md)

## What we did

Реализована система клеточного движения для игрока.

- Добавлены вспомогательные методы конвертации мировых координат в координаты клеток и обратно.
- Каждая сущность (игрок, враг) теперь знает свою текущую клетку на поле.
- На объект игрока добавлен слот для стрелочного индикатора направления.
- Центральный валидатор движения проверяет занятость целевой клетки: при свободной — перемещает сущность, при столкновении врага с игроком — уведомляет систему здоровья.
- Игрок управляется через WASD и геймпад-стик (8 направлений, включая диагонали); между ходами действует кулдаун по конфигу.
- При успешном ходе стрелочный индикатор поворачивается в направлении движения.
- При спавне/респавне игрок корректно регистрируется на карте занятости клеток.
- Отбрасывание при уроне теперь реально перемещает transform игрока в целевую клетку.
