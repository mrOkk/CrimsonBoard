...
| `updated_at` | `2026-05-24T17:57:24Z`|

## Phase History

| When | From → To | Note |
|---|---|---|
| 2026-05-24T17:57:24Z | active → done | Implemented by wf-implement |
...
Реализована система бесконечного игрового поля на основе скользящего окна чанков.

- Игровое поле делится на квадратные чанки (матрицы тайлов). Размер чанка и радиус активного окна задаются в конфиге и настраиваются из редактора Unity.
- При старте геймплея вокруг начальной позиции загружается окно активных чанков. Тайлы расставляются по сетке с чередованием двух мешей (светлый/тёмный).
- При смене чанка игрока окно сдвигается: чанки, вышедшие за пределы радиуса, очищаются и возвращаются в пул для повторного использования; новые координаты загружаются из того же пула.
- Механизм пула дополнен поддержкой callback-а инициализации, вызываемого один раз при создании нового объекта.
- Точка интеграции с системой движения игрока готова к подключению в будущих задачах.
