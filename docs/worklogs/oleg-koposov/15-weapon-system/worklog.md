...
| `updated_at` | `2026-06-03T09:25:40Z`|

## Phase History

| When | From → To | Note |
|---|---|---|
| 2026-06-03T09:25:40Z | active → done | Implemented by wf-implement |
...
- Каждое оружие теперь является отдельным префабом с привязкой по id; поле mesh удалено из конфига оружия.
- Добавлены настройки оружия: бесконечные патроны, количество патронов при подбирании, шанс выпадения при смерти врага.
- Конфиг игрока получил начальный инвентарь — список оружий со стартовым количеством патронов.
- Для каждого префаба оружия создаётся отдельный объектный пул; пулы адресуются по id оружия.
- Добавлена сущность инвентаря игрока: хранит список полученного оружия, патроны для каждого оружия и текущее активное оружие.
- Инвентарь инициализируется при спавне игрока из конфига.
- Выпавшее оружие всплывает над местом гибели врага на настраиваемой высоте; при триггере с игроком оружие подбирается, патроны начисляются, при первом подборе нового оружия происходит автоматическое переключение на него.
- При смерти врага случайно (с взвешенной вероятностью) выпадает одно оружие из тех, у кого dropChance > 0.
- В редакторе Unity поля с id оружия теперь отображаются как выпадающий список с читаемыми именами.
