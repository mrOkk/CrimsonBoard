...
| `updated_at` | `2026-05-24T19:07:11Z`|

## Phase History

| When | From → To | Note |
|---|---|---|
| 2026-05-24T19:07:11Z | active → done | Implemented by wf-implement |
...
- [x] [Task 2: OccupancyMap, GameContext, KnockbackResolver](tasks/task-2.md)
- [x] [Task 3: HealthSystem and GameplayState wiring](tasks/task-3.md)

## What we did

Реализован scaffold системы здоровья для игрока и врагов.

- Балансовые поля здоровья и урона в конфигах переведены с целого типа на дробный.
- Добавлен компонент здоровья на объекты игрока и врагов: хранит текущее и максимальное HP, не даёт HP упасть ниже нуля, генерирует событие смерти.
- Каждый враг при активации инициализируется конфигом (HP, урон) через метод Setup.
- Добавлена карта занятости клеток в общий контекст — позволяет любой системе проверить, кто стоит на данной клетке.
- Реализован детерминированный выбор клетки для отбрасывания: сначала клетка строго напротив движения врага, затем фиксированный порядок соседних клеток.
- Система здоровья принимает вызов «враг вошёл в клетку игрока», применяет урон, выполняет отбрасывание, при смерти игрока переключает FSM в GameOver; при смерти врага возвращает его в пул.
- Триггеры урона пока оставлены как TODO — ждут реализации системы движения врагов.
